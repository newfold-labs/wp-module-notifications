...
test.describe('AI Notification', () => {
  ...
  if (pluginId === 'bluehost') {
    test('appears for the six month old sites and Close button icon is clicked', async ({ page }) => {
      // Setup modal notification API intercepts
      await notifications.setupNotificationsIntercepts(page, modalNotification);
      ...
      // Wait for modal to appear (modal notifications inject HTML directly, not as notification elements)
      await page.waitForSelector('.ai-sitegen-modal', { timeout: 15000 });
      
      // Verify AI icon exists
      const aiIcon = page.locator('img[class="ai-sitegen-modal__content__right-section__heading__ai-icon"]');
      await expect(aiIcon).toHaveCount(1);
      
      // Verify modal exists and scroll to bottom
      const modal = page.locator('.ai-sitegen-modal');
      await expect(modal).toHaveCount(1);
      await modal.scrollIntoViewIfNeeded();
      
      // Verify footer elements
      const footer = page.locator('.ai-sitegen-modal__footer');
      await expect(footer).toHaveCount(1);
      
      const footerContent = page.locator('.ai-sitegen-modal__footer__content');
      await expect(footerContent).toHaveCount(1);
      
      const footerHeading = page.locator('.ai-sitegen-modal__footer__content__heading');
      await expect(footerHeading).toBeVisible();
      await expect(footerHeading).toContainText('Included FREE in your plan!');
      
      const footerButtons = page.locator('.ai-sitegen-modal__footer__content__buttons');
    ...
    test('redirects to AI onboarding when TRY NOW button is clicked', async ({ page }) => {
      ...
      const tryNowButton = page.locator('button.ai-sitegen-modal__footer__content__buttons__try-now');
      await tryNowButton.scrollIntoViewIfNeeded();
    ...
    test('modal closes when NO THANKS button is clicked', async ({ page }) => {
      ...
      const noThanksButton = page.locator('button.ai-sitegen-modal__footer__content__buttons__no-thanks');
      await noThanksButton.scrollIntoViewIfNeeded();
