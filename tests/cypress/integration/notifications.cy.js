...
describe( 'Notifications', () => {
	...
	it( 'Test notification displays in plugin app with `all`', () => {
		...
		cy.get(
			'.newfold-notifications-wrapper #notification-test-4'
	...
	it( 'Test notification displays in plugin app for specific page (settings)', () => {
		cy.get( '.newfold-notifications-wrapper #notification-test-1' ).should(
			'not.exist'
		);
	...
	it( 'Dismissing notification removes it from the page', () => {
		...
		cy.get(
			'#notification-test-2 button.notice-dismiss[data-action="close"]'
		).click();
